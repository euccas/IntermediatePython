...
from search import (get_4_letter_words, get_email, get_extension, hexadecimal,
                    tetravocalic, hexaconsonantal, possible_words,
                    five_repeats, abbreviate, palindrome5, double_double,
                    repeaters)


class Get4LetterWordsTests(unittest.TestCase):

    def test_single_word(self):
        self.assertEqual(get_4_letter_words("hiya"), ["hiya"])

    def test_three_letter_word(self):
        self.assertEqual(get_4_letter_words("hey"), [])

    def test_five_letter_word(self):
        self.assertEqual(get_4_letter_words("hello"), [])

    def test_multiple_words(self):
        sentence = "four letter word"
        words = ["four", "word"]
        self.assertEqual(get_4_letter_words(sentence), words)

    def test_capitalization(self):
        sentence = "Hi my name is Trey"
        words = ["name", "Trey"]
        self.assertEqual(get_4_letter_words(sentence), words)

    def test_punctuation(self):
        sentence = "What?!  What have you done?"
        words = ["What", "What", "have", "done"]
        self.assertEqual(get_4_letter_words(sentence), words)
...
class GetEmailTests(unittest.TestCase):
    ...
    def test_ending_in_email(self):
        sentence = "String with email 123@example.com"
        self.assertEqual(get_email(sentence), "123@example.com")

    def test_only_email(self):
        self.assertEqual(get_email("hello@domain.co"), "hello@domain.co")

    def test_punctuation_after_email(self):
        sentence = "Email is 123@example.com."
        self.assertEqual(get_email(sentence), "123@example.com")

    def test_two_emails(self):
        sentence = "abc@def.com ghi@jkl.com"
        self.assertEqual(get_email(sentence), "abc@def.com")
